...
public class LeaveGameCommand extends Command{
    ...
    protected void action(CommandData command) throws MisusedCommandException {
        ...
        try {
            if (GameManager.getGame(command.channel).removePlayer(command.author) && GameManager.getGame(command.channel).getCurrentStatus().equals(ResistanceGame.GameStatus.AWAITING_PLAYERS)) {
